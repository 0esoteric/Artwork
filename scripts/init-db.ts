...
async function initDb() {
  ...
  try {
    ...
    const runSqlFile = async (filePath: string) => {
      ...
      for (const statement of statements) {
        try {
          ...
          if (err.code === 'ER_DUP_KEY' || err.code === 'ER_DUP_ENTRY' || err.code === 'ER_DUP_KEYNAME' || err.code === 'ER_DUP_FIELDNAME') {
            // Ignore duplicate errors during seeding or re-init
            continue
          }
    ...
    await runSqlFile(path.join(scriptsDir, '003-add-product-fields.sql'))
