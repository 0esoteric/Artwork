    ...
    for (const product of sampleProducts) {
      ...
      if (product.artistName) {
        ...
        if (artists.length) {
          ...
          const artistSlug = product.artistName.toLowerCase().replace(/\s+/g, '-')
          const [result] = await connection.execute(
            "INSERT INTO artists (name, slug, bio, created_at, updated_at) VALUES (?, ?, ?, NOW(), NOW())",
            [product.artistName, artistSlug, `Skilled artisan specializing in ${product.categorySlug.replace("-", " ")}`]
          ) as any
      ...
      const [result] = await connection.execute(
        `INSERT INTO products 
         (name, slug, description, short_description, category_id, price, compare_price, sku, stock_quantity, artist_id, dimensions, medium, art_form, is_active, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`,
        [
          product.name,
          product.slug,
